...
from rng import GameSeeds, ItemsSeeds, PickupSeeds1, PickupSeeds2, CollectiblesSeeds, TrinketsSeeds, PillsSeeds, \
	CardsSeeds
...
class Game:
	...
	def generate_individual_rng(self):
		...
		self.pills_seeds = PillsArray(PillsSeeds(self.items_seeds.next(), self.verbose), self.verbose)
		...
		self.cards_seeds = CardsArray(CardsSeeds(self.items_seeds.next(), self.verbose), self.verbose)
